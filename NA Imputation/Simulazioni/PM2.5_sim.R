...
SDEM_w_gl <- function(data) {
  ...
  tr <- data %>% 
    ...
    mutate(w_eff = w_eff_gen(week, AirQualityStationEoICode), 
           ...
           h_eff = h_eff_gen(hour, AirQualityStationEoICode), 
           x_mean = x_mean_gen(week, day, hour, AirQualityStationEoICode)) %>%
    select(PM2.5, x_mean, w_eff, d_eff, h_eff) 
  
  #Calcolo dei pesi che verranno usati in fase di imputazione
  pesi <- lm(PM2.5 ~ 0+., data = tr)$coeff
...
valutazione_sim <- function(x) {
  ...
  imps <- SDEM_pm2.5(may_data_na)
  imps <- imps %>% filter(DatetimeBegin < (h_times[1] + hours(6))) %>% pull(PM2.5)
  may_data_na[may_data_na$DatetimeBegin < (h_times[1] + hours(6)) ,"PM2.5"] <- imps
  ...
  tic()
  may_data_na_SDEM <- SDEM_pm2.5(may_data_na_SDEM)
  tot_time <- tictoc::toc(quiet = T)
  tot_time <- tot_time$toc - tot_time$tic
  
  yhat <- may_data_na_SDEM[x,"PM2.5"] %>% pull()
  y <- true[,"PM2.5"] %>% pull()
  ...
  yhat <- may_data_na_SDEM_w_gl[x,"PM2.5"] %>% pull()
  y <- true[,"PM2.5"] %>% pull()
  ...
  yhat <- may_data_na_SDEM_w_st[x,"PM2.5"] %>% pull()
  y <- true[,"PM2.5"] %>% pull()
  ...
  for (s in 1:length(st)) {
    ...
    fit_st <- mgcv::gam(PM2.5_t ~ PM2.5_t_1 +
                          PM2.5_t_2 +
                          PM2.5_t_3 +
                          PM2.5_t_4 +
                          PM2.5_t_5 +
    ...
    fit_st_fut <- mgcv::gam(PM2.5_t ~ PM2.5_t_1 +
                              PM2.5_t_2 +
                              PM2.5_t_3 +
                              PM2.5_t_4 +
                              PM2.5_t_5 +
  ...
  for (i in (inizio):length(d_times)) {
    ...
    na_data <- ts_data %>% filter(lubridate::as_date(DatetimeBegin) == d_times[i]) %>% filter(is.na(PM2.5_t))
    ...
    if(nrow(na_data) > 0) {
      ...
      for (k in 1:length(h_day)) {
        ...
        may_data_na_mod_st_fut[which(is.na(may_data_na_mod_st_fut$PM2.5) & may_data_na_mod_st_fut$DatetimeBegin == h_day[k]), "PM2.5"] <- prev
        ts_data <- may_data_na_mod_st_fut  %>% make_ts() %>% mutate(time = (wday(DatetimeBegin) - 1) * 24 + (hour(DatetimeBegin) + 1))
        na_data <- ts_data %>% filter(lubridate::as_date(DatetimeBegin) == d_times[i]) %>% filter(is.na(PM2.5_t))
  ...
  yhat <- may_data_na_mod_st_fut[x,"PM2.5"] %>% pull()
  y <- true[,"PM2.5"] %>% pull()
  ...
  mod <- spgwr::gwr(PM2.5_t ~ PM2.5_t_1 +
                      PM2.5_t_2 +
                      PM2.5_t_3 +
                      PM2.5_t_4 +
                      PM2.5_t_5 +
  ...
  mod_fut <- spgwr::gwr(PM2.5_t ~ PM2.5_t_1 +
                          PM2.5_t_2 +
                          PM2.5_t_3 +
                          PM2.5_t_4 +
                          PM2.5_t_5 +
      ...
      for (k in 1:length(h_day)) {
        ...
        for (h in 1:nrow(na_h)) {
          ...
          if(!is.na(na_h[h,]$PM2.5_t1) & !is.na(na_h[h,]$PM2.5_t2) & !is.na(na_h[h,]$PM2.5_t3) & !is.na(na_h[h,]$PM10_t1) & !is.na(na_h[h,]$PM10_t2) & !is.na(na_h[h,]$PM10_t3)) {
            ...
            pred <- spgwr::gwr(PM2.5_t ~ PM2.5_t_1 +
                                 PM2.5_t_2 +
                                 PM2.5_t_3 +
                                 PM2.5_t_4 +
                                 PM2.5_t_5 +
        ...
        may_data_na_mod_gwr_fut[which(is.na(may_data_na_mod_gwr_fut$PM2.5) & may_data_na_mod_gwr_fut$DatetimeBegin == h_day[k]), "PM2.5"] <- prev
        ts_data <- may_data_na_mod_gwr_fut  %>% make_ts() %>% mutate(time = (wday(DatetimeBegin) - 1) * 24 + (hour(DatetimeBegin) + 1))
        na_data <- ts_data %>% filter(lubridate::as_date(DatetimeBegin) == d_times[i]) %>% filter(is.na(PM2.5_t))
  ...
  yhat <- may_data_na_mod_gwr_fut[x,"PM2.5"] %>% pull()
  y <- true[,"PM2.5"] %>% pull()
...
res_pm2.5 <- future.apply::future_lapply(X = sims, FUN = valutazione_sim)
