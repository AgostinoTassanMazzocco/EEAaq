...
SDEM_w_gl <- function(data) {
  ...
  pesi <- lm(`Black Carbon` ~ 0+., data = tr)$coeff
...
SDEM_w_st <- function(data) {
  ...
  tr <- data %>% 
    ...
    mutate(w_eff = w_eff_gen(week, AirQualityStationEoICode), 
           ...
           h_eff = h_eff_gen(hour, AirQualityStationEoICode), 
           x_mean = x_mean_gen(week, day, hour, AirQualityStationEoICode)) %>%
    select(AirQualityStationEoICode,`Black Carbon`, x_mean, w_eff, d_eff, h_eff) 
  ...
  pesi <- tr %>% group_by(AirQualityStationEoICode) %>% 
    reframe(pesi = coefficients(lm(`Black Carbon` ~ 0+ .data$x_mean + .data$w_eff + .data$d_eff + .data$h_eff))) %>%
...
valutazione_sim <- function(x) {
  ...
  imps <- imps %>% filter(DatetimeBegin < (h_times[1] + hours(6))) %>% pull(`Black Carbon`)
  may_data_na[may_data_na$DatetimeBegin < (h_times[1] + hours(6)) ,"Black Carbon"] <- imps
  ...
  tic()
  may_data_na_SDEM <- SDEM_bc(may_data_na_SDEM)
  tot_time <- tictoc::toc(quiet = T)
  tot_time <- tot_time$toc - tot_time$tic
  
  yhat <- may_data_na_SDEM[x,"Black Carbon"] %>% pull()
  ...
  yhat <- may_data_na_SDEM_w_gl[x,"Black Carbon"] %>% pull()
  y <- true[,"Black Carbon"] %>% pull()
  ...
  yhat <- may_data_na_SDEM_w_st[x,"Black Carbon"] %>% pull()
  y <- true[,"Black Carbon"] %>% pull()
  ...
  for (s in 1:length(st)) {
    ...
    train_st_fut <- train_st %>% filter(!is.na(BC_t1) & !is.na(BC_t2) & !is.na(BC_t3) & !is.na(PM2.5_t1) & !is.na(PM2.5_t2) & !is.na(PM2.5_t3))
    
    
    fit_st_fut <- mgcv::gam(BC_t ~ BC_t_1 +
                              BC_t_2 +
                              BC_t_3 +
                              BC_t_4 +
                              BC_t_5 +
    ...
    if(nrow(na_data) > 0) {
      ...
      for (k in 1:length(h_day)) {
        ...
        for (h in 1:nrow(na_h)) {
          ...
          if(!is.na(na_h[h,]$BC_t1) & !is.na(na_h[h,]$BC_t2) & !is.na(na_h[h,]$BC_t3) & !is.na(na_h[h,]$PM2.5_t1) & !is.na(na_h[h,]$PM2.5_t2) & !is.na(na_h[h,]$PM2.5_t3)) {
        ...
        may_data_na_mod_st_fut[which(is.na(may_data_na_mod_st_fut$`Black Carbon`) & may_data_na_mod_st_fut$DatetimeBegin == h_day[k]), "Black Carbon"] <- prev
        ts_data <- may_data_na_mod_st_fut  %>% make_ts() %>% mutate(time = (wday(DatetimeBegin) - 1) * 24 + (hour(DatetimeBegin) + 1))
        na_data <- ts_data %>% filter(lubridate::as_date(DatetimeBegin) == d_times[i]) %>% filter(is.na(BC_t))
  ...
  yhat <- may_data_na_mod_st_fut[x,"Black Carbon"] %>% pull()
  y <- true[,"Black Carbon"] %>% pull()
  ...
  train <- ts_data %>% filter(DatetimeBegin >= d_times[inizio] + hours(6)) %>% 
    filter(!is.na(BC_t) & !is.na(BC_t_1) & !is.na(BC_t_2) & !is.na(BC_t_3) &!is.na(BC_t_4) & !is.na(BC_t_5) & !is.na(PM2.5_t) & !is.na(PM2.5_t_1) & !is.na(PM2.5_t_2) & !is.na(PM2.5_t_3) &!is.na(PM2.5_t_4) & !is.na(PM2.5_t_5))
  
  train_fut <- train %>% filter(!is.na(BC_t1) & !is.na(BC_t2) & !is.na(BC_t3) ^ !is.na(PM2.5_t1) & !is.na(PM2.5_t2) & !is.na(PM2.5_t3))
  ...
  mod <- spgwr::gwr(BC_t ~ BC_t_1 +
                      ...
                      PM2.5_t_4 +
                      PM2.5_t_5,
                    data = train, bandwidth = 0.3)
  
  mod_fut <- spgwr::gwr(BC_t ~ BC_t_1 +
                          BC_t_2 +
                          BC_t_3 +
                          BC_t_4 +
                          BC_t_5 +
  ...
  for (i in (inizio):length(d_times)) {
    ...
    if(nrow(na_data) > 0) {
      ...
      for (k in 1:length(h_day)) {
        ...
        may_data_na_mod_gwr_fut[which(is.na(may_data_na_mod_gwr_fut$`Black Carbon`) & may_data_na_mod_gwr_fut$DatetimeBegin == h_day[k]), "Black Carbon"] <- prev
        ts_data <- may_data_na_mod_gwr_fut  %>% make_ts() %>% mutate(time = (wday(DatetimeBegin) - 1) * 24 + (hour(DatetimeBegin) + 1))
        na_data <- ts_data %>% filter(lubridate::as_date(DatetimeBegin) == d_times[i]) %>% filter(is.na(BC_t))
  ...
  yhat <- may_data_na_mod_gwr_fut[x,"Black Carbon"] %>% pull()
  y <- true[,"Black Carbon"] %>% pull()
...
res_bc <- future.apply::future_lapply(X = sims, FUN = valutazione_sim)
