...
#Mod mensile
#GWR
#Provo con osservazioni future (da t+1 a t+3)
...
for (j in 1:50) {
  ...
  #imps <- imps %>% filter(DatetimeBegin < (d_times[1] + days(14))) %>% pull(PM2.5)
  #may_data_na[may_data_na$DatetimeBegin < (d_times[1] + days(14)) ,"PM2.5"] <- imps
  ...
  tic()
  mod <- GWmodel::gtwr(PM2.5_t ~ PM2.5_t_1 + 
                         PM2.5_t_2 +
                         PM2.5_t_3 +
                         PM2.5_t_4 +
                         PM2.5_t_5 +
                         ...
                         PM10_t_3 +
                         PM10_t_4 +
                         PM10_t_5, data = train, obs.tv = train$time, st.bw = 0.2)
  toc()
      ...
      for (k in 1:length(h_day)) {
        ...
        for (h in 1:nrow(na_h)) {
          ...
          if(!is.na(na_h[h,]$PM2.5_t1) & !is.na(na_h[h,]$PM2.5_t2) & !is.na(na_h[h,]$PM2.5_t3) & !is.na(na_h[h,]$PM10_t1) & !is.na(na_h[h,]$PM10_t2) & !is.na(na_h[h,]$PM10_t3)) {
            ...
            prev <- spgwr::gwr(PM2.5_t ~ PM2.5_t_1 + 
                                 PM2.5_t_2 +
                                 PM2.5_t_3 +
                                 PM2.5_t_4 +
                                 PM2.5_t_5 +
        ...
        may_data_na[which(is.na(may_data_na$PM2.5) & may_data_na$DatetimeBegin == h_day[k]), "PM2.5"] <- prev_BC
...
mod <- GWmodel::gtwr(PM2.5_t ~ PM2.5_t_1 + 
                       ...
                       PM10_t_3 +
                       PM10_t_4 +
                       PM10_t_5, data = train, obs.tv = time, 
                     bandwidth = 0.2)


col.bw <- GWmodel::bw.gtwr(PM2.5_t ~ PM2.5_t_1 + 
                             PM2.5_t_2 +
                             PM2.5_t_3 +
                             PM2.5_t_4 +
                             PM2.5_t_5 +
                             ...
                             PM10_t_3 +
                             PM10_t_4 +
                             PM10_t_5, data = train, obs.tv = train$time)
