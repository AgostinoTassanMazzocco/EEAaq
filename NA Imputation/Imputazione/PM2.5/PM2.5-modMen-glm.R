...
#1 modello al mese per ogni stazione
#Aggiungo le osservazioni future


#######################################################
#imputazione pm2.5 usando pm10
#######################################################
...
for (j in 1:50) {
  ...
  for (s in 1:length(st)) {
    ...
    tic()
    fit_BC_st <- mgcv::gam(PM2.5_t ~ PM2.5_t_1 + 
                             PM2.5_t_2 +
                             PM2.5_t_3 +
                             PM2.5_t_4 +
                             PM2.5_t_5 +
    ...
    tic()
    fit_BC_st_fut <- mgcv::gam(PM2.5_t ~ PM2.5_t_1 + 
                                 PM2.5_t_2 +
                                 PM2.5_t_3 +
                                 PM2.5_t_4 +
                                 PM2.5_t_5 +
  ...
  for (i in (inizio):length(d_times)) {
    ...
    if(nrow(na_data) > 0) {
      ...
      for (k in 1:length(h_day)) {
        ...
        #prev_BC <- predict(fit_BC, na_h)$predictions
