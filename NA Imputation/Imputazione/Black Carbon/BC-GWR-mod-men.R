...
for (j in 1:10) {
  ...
  train <- ts_data %>% filter(DatetimeBegin >= d_times[inizio] + hours(6)) %>% 
    filter(!is.na(BC_t) & !is.na(BC_t_1) & !is.na(BC_t_2) & !is.na(BC_t_3) &!is.na(BC_t_4) & !is.na(BC_t_5)) 
  train_fut <- train %>% filter(!is.na(BC_t1) & !is.na(BC_t2) & !is.na(BC_t3))
    ...
    if(nrow(na_data) > 0) {
      ...
      for (k in 1:length(h_day)) {
        ...
        for (h in 1:nrow(na_h)) {
          ...
          if(!is.na(na_h[h,]$BC_t1) & !is.na(na_h[h,]$BC_t2) & !is.na(na_h[h,]$BC_t3)) {
            
            prev <- spgwr::gwr(BC_t ~ BC_t_1 + 
                                 BC_t_2 +
                                 BC_t_3 +
                                 BC_t_4 +
                                 BC_t_5 +
                                 ...
                                 PM10_t2 +
                                 PM10_t3, data = train_fut, 
                               bandwidth = 0.478, fit.points = na_h[h,], predictions = TRUE, fittedGWRobject = mod_fut)
            ...
            prev <- spgwr::gwr(BC_t ~ BC_t_1 + 
                                 ...
                                 PM10_t_4 + 
                                 PM10_t_5, data = train, 
                               bandwidth = 0.478, fit.points = na_h[h,], predictions = TRUE, fittedGWRobject = mod)
            prev_BC[h] <- prev$SDF$pred
...
col.bw <- spgwr::ggwr.sel(BC_t ~ BC_t_1 + 
                            BC_t_2 +
                            BC_t_3 +
                            BC_t_4 +
                            BC_t_5, data = train)
