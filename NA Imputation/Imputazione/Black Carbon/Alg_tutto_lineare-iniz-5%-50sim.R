...
for (j in 1:1) {
  ...
  #ore:
  #train <- ts_data %>% 
  #  filter(DatetimeBegin >= h_times[inizio] - hours(168) & DatetimeBegin <= h_times[inizio] - hours(1))
  #na_data <- ts_data %>% filter(DatetimeBegin == h_times[inizio]) %>% filter(is.na(BC_t))
  #giorni:
  train <- ts_data %>% 
    filter(DatetimeBegin >= d_times[inizio] - days(7) & DatetimeBegin <= d_times[inizio] - hours(1))
  na_data <- ts_data %>% filter(lubridate::as_date(DatetimeBegin) == d_times[inizio]) %>% filter(is.na(BC_t))
  
  #sum(is.na(na_data$BC_t_1))
  #na_data %>% select(DatetimeBegin, BC_t, BC_t_1, BC_t_24) %>% print(n = Inf)
  ...
  time_base <- tictoc::toc(quiet = T)
  time_base <- time_base$toc - time_base$tic
  time_prev <- 0
  ...
  #na_data %>% select(AirQualityStationEoICode, DatetimeBegin, BC_t, BC_t_1) %>% print(n = Inf)
  ...
  if(nrow(na_data) > 0) {
    ...
    #prev_BC <- exp(predict(fit_BC, filter(na_data, is.na(BC_t))))
    #may_data_na[which(is.na(may_data_na$`Black Carbon`) & lubridate::as_date(may_data_na$DatetimeBegin) == d_times[inizio]), "Black Carbon"] <- na_data$BC_t
  }
  
  
  #filter(na_data, is.na(BC_t)) %>% print(n = Inf) %>% select(AirQualityStationEoICode, PM10_t, PM2.5_t, BC_t_1, BC_t_24)
  #may_data_na %>% filter(is.na(`Black Carbon`) & as_date(DatetimeBegin) == as_date("2019-05-24"))
  
  
  for (i in (inizio+1):length(d_times)) {
    tic()
    #cat(paste0("Imputing data for ", h_times[i], "\n"))
    ts_data <- may_data_na  %>% make_ts() %>% mutate(time = (wday(DatetimeBegin) - 1) * 24 + (hour(DatetimeBegin) + 1))
    train <- ts_data %>% filter(DatetimeBegin >= d_times[i] - hours(167) & DatetimeBegin <= d_times[i] - hours(1))
    na_data <- ts_data %>% filter(lubridate::as_date(DatetimeBegin) == d_times[i]) %>% filter(is.na(BC_t))
    
    
    
    #Inizializzazione modello per la prima ora
    init_coef_BC <- coef(fit_BC)
    init_sp_BC <- list(sp = fit_BC$sp, scale = 0)
    ...
    if(nrow(na_data) > 0) {
      ...
      if(time_prev < time_base) {
        ...
        fit_BC <- mgcv::gam(BC_t ~ s(time) + 
                              ...
                              AirQualityStationType, 
                            data = train, family = Gamma(link = "log"), 
                            in.out = init_sp_BC, start = init_coef_BC)
        time_prev <- tictoc::toc(quiet = T)
        time_prev <- time_prev$toc - time_prev$tic
