...
#w_eff_gen(data$week[1], data$AirQualityStationEoICode[1]) #0.0241
#d_eff_gen(data$day[1], data$AirQualityStationEoICode[1])
#data %>% rowwise() %>% mutate(x_mean = x_mean_gen(week, day, hour, AirQualityStationEoICode)) %>% select(x_mean)
#data %>% rowwise() %>% mutate(w_eff = w_eff_gen(week, AirQualityStationEoICode), 
#                              d_eff = d_eff_gen(day, AirQualityStationEoICode), 
#                              h_eff = h_eff_gen(hour, AirQualityStationEoICode)) %>% 
#  select(w_eff, d_eff, h_eff)
...
SDEM <- function(data) {
  ...
  #  
  #  
  #  return(w[1]x_mean + w[2]*w_eff + w[3]*d_eff + w[4]*h_eff)
  #  
  #  
  #}
